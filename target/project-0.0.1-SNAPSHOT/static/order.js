...
function mainOrdersListTable(data){
	...
	for(var i in data){
		var e = data[i];
		if(e.invoiceCreated){
			var buttonHtml='<button class="btn btn-info" id="insideEdit" disabled >View & Edit</button>';
		}else{
...
function createOrderItemsHtml(data,id) {
	...
	$tbody.append()
	table.append('<tr><td colspan="3" style="font-weight:700;padding-left:3%; font-size:22px"> Total: '+total+'</td><td colspan="3"><button class="btn btn-warning insideEdit" style="margin-left:66%;" onclick="displayAddOrderItemModal(' + id + ')"><i class="fa fa-plus" aria-hidden="true"></i> Add Item</button></td></tr>');
}
